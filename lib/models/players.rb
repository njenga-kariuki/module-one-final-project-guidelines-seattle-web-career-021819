...
class Player
  def self.search_player_api_return_id(player_name)
    #call api
  response_string = RestClient.get("https://www.balldontlie.io/api/v1/players?&search=#{player_name[:last_name]}")
  response_hash = JSON.parse(response_string)

    #iterate through response and check for match and return player
    player_hash = response_hash["data"].select do |player|
      player["first_name"] == player_name[:first_name] &&
      player["last_name"] == player_name[:last_name]
    end
